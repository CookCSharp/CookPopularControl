...
namespace CookPopularCSharpToolkit.Communal
{
    ...
    public static class StringsExtension
    {
        ...
        public static string ToOriginFromHexString(this string asciiStr)
        {
            try
            {
                if (string.IsNullOrEmpty(asciiStr)) return default;
                var bytes = new byte[asciiStr.Length / 2];
                for (int i = 0; i < asciiStr.Length / 2; i++)
                {
                    bytes[i] = byte.Parse(asciiStr.Substring(i * 2, 2), System.Globalization.NumberStyles.HexNumber);
                }
